...
def test_parse_mmlu_response():
    ...
    model_output = (
        "The correct answer is B. There is only one human polyomavirus known at present, which is the BK virus."
    )
    parsed_response = run_parse_mmlu_response(mmlu_example=mmlu_example, model_output=model_output)
...
def test_parse_gsm8k_response_unknown():
    model_output = (
        "Natalia sold twenty-four clips in May. Thus, Natalia sold seventy-two clips altogether in April and May."
    )
    parsed_response = run_parse_gsm8k_response(model_output=model_output)
    assert not parsed_response
